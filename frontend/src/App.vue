...
const menuItems = [
  {
    ...
    children: [
      ...
      { label: "대출", path: "/products?group=loan" },
      { label: "연금저축", path: "/products?group=savings" }
    ]
  },
  {
    label: "금융상품",
...
onBeforeUnmount(() => clearCloseDropdownTimer());
...
<template>
  <div class="app-shell">
    <header class="topbar finpick-topbar" @mouseenter="clearCloseDropdownTimer" @mouseleave="scheduleCloseDropdown">
      <div class="header-inner">
        ...
        <nav class="finpick-nav" :class="{ open: mobileOpen, 'has-open-dropdown': hasOpenDropdown }" aria-label="주요 메뉴" @focusout="handleNavFocusout">
          <div
            ...
            @mouseleave="item.children && scheduleCloseDropdown()"
          >
