...
const favorites = computed(() => [
  ...(auth.user?.joined_products || []).map(product => ({
    key: `product:${product.id}`,
    group: "product",
    id: product.id,
    typeLabel: product.product_type_label,
    name: product.name,
    bankName: product.bank_name,
    rateText: `최고 ${product.best_rate || 0}%`,
    detailTo: `/products/${product.id}`,
    removePath: `/products/${product.id}/join/`
  })),
  ...(auth.user?.joined_loans || []).map(loan => ({
    key: loan.favorite_key || `loan:${loan.loan_type}:${loan.product_code}`,
    group: "loan",
    ...
    rateText: loan.rate_min || loan.rate_min === 0 ? `최저 ${Number(loan.rate_min).toFixed(2)}%` : "금리 정보 확인",
    detailTo: `/products/loan/${encodeURIComponent(loan.loan_type)}/${encodeURIComponent(loan.product_code)}`,
    removePath: `/loans/${encodeURIComponent(loan.loan_type)}/${encodeURIComponent(loan.product_code)}/join/`
  })),
  ...(auth.user?.joined_savings || []).map(item => ({
    key: item.favorite_key || `savings:${item.product_code}`,
    group: "savings",
    productCode: item.product_code,
    typeLabel: item.product_type_label || "저축상품",
    name: item.name,
    bankName: item.bank_name,
    rateText: item.rate_value || item.rate_value === 0 ? `최고 ${Number(item.rate_value).toFixed(2)}%` : "수익률 정보 확인",
    detailTo: `/products/savings/${encodeURIComponent(item.product_code)}`,
    removePath: `/savings/${encodeURIComponent(item.product_code)}/join/`
  }))
]);
...
async function removeFavorite(item) {
  ...
  error.value = "";
  try {
    const user = await apiFetch(item.removePath, { method: "DELETE" });
    auth.setUser(user);
  } catch (err) {
...
<template>
  <main class="container favorites-page-local">
    ...
    <section v-if="favorites.length" class="favorite-grid-local">
      <article v-for="item in favorites" :key="item.key" class="favorite-card-local">
        <div class="favorite-card-top-local">
          <span class="favorite-type-local" :class="{ loan: item.group === 'loan', savings: item.group === 'savings' }">{{ item.typeLabel }}</span>
          <button type="button" :disabled="removingKey === item.key" @click="removeFavorite(item)">
            {{ removingKey === item.key ? "해제 중" : "♥ 관심 해제" }}
          </button>
        </div>
        <h2>{{ item.name }}</h2>
        <p>{{ item.bankName }}</p>
        <div class="favorite-card-bottom-local">
          <strong>{{ item.rateText }}</strong>
          <RouterLink :to="item.detailTo">상품 상세보기 <span>›</span></RouterLink>
        </div>
      </article>
    </section>
...
.favorite-type-local.loan { background: #e8f1ff; color: #1f66d6; }
.favorite-type-local.savings { background: #e9f8f5; color: #16806f; }
...
.favorite-card-bottom-local { display: flex; align-items: center; justify-content: space-between; gap: .75rem; border-top: 1px dashed #dbe5f0; margin-top: 1rem; padding-top: .85rem; }
.favorite-card-bottom-local strong { color: #176be9; font-size: 1.25rem; letter-spacing: -.04em; }
.favorite-card-bottom-local a { border: 1px solid #bdd3ff; border-radius: 8px; color: #246fe5; font-size: .78rem; font-weight: 850; padding: .38rem .55rem; text-decoration: none; white-space: nowrap; }
...
@media (max-width: 640px) { .favorites-page-local { padding-top: 1.75rem; } .favorites-hero-local { align-items: start; flex-direction: column; } .favorite-grid-local { grid-template-columns: 1fr; } .favorite-card-bottom-local { align-items: start; flex-direction: column; } }
</style>
